#!/usr/bin/env python3
"""Qwen multi-token gen with bias + repetition penalty."""
...
SEQ_LEN = 32; HIDDEN = 896; PAD = 151643
...
embed = np.load(args.embed); bias = np.load(args.bias); K = args.k
...
print(f"prompt: {args.prompt!r}"); print(args.prompt, end="", flush=True)
total = 0.0
for _ in range(args.tokens):
    ...
    for tid in generated_recent:
        logits[tid] = logits[tid]/args.rep_penalty if logits[tid]>0 else logits[tid]*args.rep_penalty
    if args.top_k > 0:
        idx = logits.argsort()[-args.top_k:]
        probs = np.exp((logits[idx]-logits[idx].max())/args.temperature); probs /= probs.sum()
    ...
    if len(generated_recent)>8: generated_recent=generated_recent[-8:]
