...
M1 전체 README + 재현 스크립트 정리, 시간순 커밋으로 반영.

---

## 다음

- M1 마무리: NB를 디바이스에 push → vpm_run 검증 → FP32 vs uint8 KL divergence 측정 → tok/s 실측
- M2: Qwen2.5-0.5B 한국어 파일럿 (SmoothQuant + KV cache 도입)
